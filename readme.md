...
Дополнительных действий не требуется. В example.env уже заполнены все данные для проверки тестового задания, включая токен WB, сервисный аккаунт Google и ID таблицы. Compose автоматически загружает example.env, применяет миграции и запускает cron-задачу.
...
Работа со своими данными (production)

Если вы хотите использовать собственные токены, таблицы и базу, создайте файл .env:

```
cp example.env .env
```

Заполните .env своими значениями: поставьте NODE_ENV=production, укажите свой WB_API_TOKEN, SPREADSHEET_IDS и GOOGLE_SERVICE_ACCOUNT_JSON. Затем запустите ту же команду:

```
docker compose up --build
```

Compose загружает example.env как базовые значения, а .env (если он есть) перезаписывает их сверху. В production-режиме seeds не запускаются.
